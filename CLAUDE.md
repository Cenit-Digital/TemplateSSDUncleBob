...
> Esta plantilla implementa el **arnés SSD de Uncle Bob (edición artesano)**:
> conversación → Gherkin → TDD → review → mutación. Ver `docs/workflow.md`.
> Referencia trabajada end-to-end en `examples/notes-cli/`; el método, en `docs/`.
...
> **Stack.** Python es el stack de referencia del arnés. Para trabajar en
> otro lenguaje, lee `docs/stack-adapter.md`: la capa de adaptación mapea
> `init.sh`, la ejecución de tests y `tools/mutate.py` a tu stack sin romper
> el método.
