...
3. **Por cada escenario `@s` en orden**, ejecuta uno o más ciclos
   ...
   a. **ROJO** — escribe un test en `tests/` que codifica ese Given/When/
      Then y verifica que **falla** (`python3 -m unittest …` — comando de
      tests del proyecto; Python es la referencia, ver
      docs/stack-adapter.md). Un test que pasa a la primera no demuestra
      nada: ajústalo o sospecha.
